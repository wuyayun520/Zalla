...
// __CLOSE_PRINT__
//
//  BranchReactiveCompatible.swift
//  AbroadTalking
//
...
class BranchReactiveCompatible: MusicViewController {
    //: var type = LoginType.PhoneLogin
    var type = PublishTitleConvertible.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    override func viewDidLoad() {
        ...
        restrictionChangeSourceEqual()
        //: bindInteraction()
        primaryColorTotaleraction()
    }

    // MARK: - Lazy load

    //: private lazy var pwdView: TalkingSetNewPwdView = {
    private lazy var pwdView: MixedReactiveCompatible = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = MixedReactiveCompatible(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension BranchReactiveCompatible {
    /// 修改密码
    //: private func req_modifyPwd(_ pwd: String, _ vcode: String?) {
    private func page(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        AtAdView.sizeUp()
        //: TalkingLoginRequestTool.req_modifyPassword(pwd: pwd, vcode: vcode) { [weak self] succeed, result, errorModel in
        WhiteIndexReactiveCompatible.buildEnable(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            AtAdView.toAction()
    ...
    private func moment() {
        ...
        WhiteIndexReactiveCompatible.cast { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            AtAdView.toAction()
            //: guard let self = self else { return }
            guard let self = self else { return }
...
extension BranchReactiveCompatible {
    ...
    private func restrictionChangeSourceEqual() {
    ...
    private func primaryColorTotaleraction() {
