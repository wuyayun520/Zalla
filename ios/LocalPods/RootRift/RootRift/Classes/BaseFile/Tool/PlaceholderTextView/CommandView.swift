...
/*: / :*/
fileprivate let str_infoText:String = "label"

/*: "\n" :*/
fileprivate let str_indexValue:[Character] = ["\n"]
...
struct RepublishContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class CommandView: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        postError(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        postError(placeholder: nil, placeholderColor: nil)
    }
}
...
extension CommandView {
    ...
    private func postError(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(WriteLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(WriteLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-WriteLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: RepublishContainerInset().top, left: RepublishContainerInset().left, bottom: RepublishContainerInset().bottom, right: RepublishContainerInset().right)
...
extension CommandView: UITextViewDelegate {
...
extension CommandView {
    ...
    private func hiddenPlaceholder() {
