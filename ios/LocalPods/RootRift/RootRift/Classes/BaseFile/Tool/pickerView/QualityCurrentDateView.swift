...
/*: "Select a date" :*/
fileprivate let str_titleValue:[Character] = ["S","e","l","e","c","t"," ","a"," ","d","a","t","e"]

/*: M" :*/
fileprivate let str_requestName:[Character] = ["M"]

/*: D" :*/
fileprivate let str_clearValue:String = "cell"

/*: Y" :*/
fileprivate let str_appName:String = "path"
...
class QualityCurrentDateView: CountryView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func weltanschauungAt() {
        //: super.initPickView()
        super.weltanschauungAt()

        //: titleString = "Select a date".localized
        titleString = (String(str_titleValue)).localized
        ...
        tillSecond(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func tillSecond(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func snapSizeBackground(button: UIButton) {
        //: super.confirmClick(button: button)
        super.snapSizeBackground(button: button)
        //: refreshPickViewData()
        takeTo()
        ...
        if let day = selectDay {
            ...
            delegate?.version(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.version(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return channelStreetSmart(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func channelStreetSmart(year: Int, month: Int) -> Int {
