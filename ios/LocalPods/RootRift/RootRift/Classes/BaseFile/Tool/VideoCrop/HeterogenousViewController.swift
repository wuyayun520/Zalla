...
fileprivate let str_myName:[UInt8] = [0x38,0x3f,0x38,0x25,0x79,0x32,0x3e,0x35,0x34,0x23,0x6b,0x78,0x71,0x39,0x30,0x22,0x71,0x3f,0x3e,0x25,0x71,0x33,0x34,0x34,0x3f,0x71,0x38,0x3c,0x21,0x3d,0x34,0x3c,0x34,0x3f,0x25,0x34,0x35]

private func pageSegment(content num: UInt8) -> UInt8 {
    return num ^ 81
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let str_tempTitle:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t","i","h","w","_","k","c","a","b","_","s","h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let str_sufficientData:String = "Editsize center self"

/*: "Finish" :*/
fileprivate let str_shareValue:String = "view make nameFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let str_insideValue:[Character] = ["b","t","n","Y","D","o"]
fileprivate let str_labFemaleLiveData:String = "2"
fileprivate let str_buttonContent:String = "AS_ofor entire block var self"
fileprivate let str_errorData:String = "plastack"

/*: "Cancel" :*/
fileprivate let str_videoText:String = "Cancelof import index message"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let str_headData:String = "bgQkRguard let"
fileprivate let str_productTitle:String = "etov"
fileprivate let str_translateName:String = "share hidden_toast"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let str_viewValue:[UInt8] = [0x1e,0x32,0x2f,0xea,0x40,0x33,0x2e,0x2f,0x39,0xea,0x36,0x2f,0x38,0x31,0x3e,0x32,0xea,0x3d,0x32,0x39,0x3f,0x36,0x2e,0xea,0x2c,0x2f,0xea,0xef,0xa,0xf7,0xef,0xa,0xea,0x3d,0x2f,0x2d,0x39,0x38,0x2e,0x3d,0xf8,0xea,0x1a,0x36,0x2f,0x2b,0x3d,0x2f,0xea,0x33,0x38,0x3e,0x2f,0x3c,0x2d,0x2f,0x3a,0x3e,0xea,0x33,0x3e]

fileprivate func errorMake(center num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  HeterogenousViewController.swift
//  AbroadTalking
//
...
typealias VideoEditBlock = (_ videoInfo: MusicModelType?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kLet_maleName: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class HeterogenousViewController: MusicViewController {
    ...
    private var slider: MechanicallyThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = MusicModelType() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = RawTalkingEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if NameSocketManager.shared.isTalking == true { // 正在音视频通话，静音播放
    ...
    init(style: RawTalkingEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kLet_scaleName - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_myName.map{pageSegment(content: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.withView(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.withView(name: (String(str_tempTitle))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = FactoryShowThen.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(equalAction), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(str_sufficientData.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.smallFor(), for: .normal)
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(str_shareValue.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.showSize(fontSize: 14)
        v.titleLabel?.font = UIFont.showSize(fontSize: 14)
        ...
        v.backgroundColor = UIColor.smallFor()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(positionPathEffect), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.withView(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.withView(name: (String(str_insideValue) + str_labFemaleLiveData.capitalized + String(str_buttonContent.prefix(4)) + "ediv_" + str_errorData.replacingOccurrences(of: "stack", with: "y"))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(courseOfActionDataSender(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(str_videoText.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(performanceAction), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.withView(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.withView(name: (String(str_headData.prefix(5)) + "TbF_o" + str_productTitle.replacingOccurrences(of: "to", with: "di") + String(str_translateName.suffix(6)))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: str_viewValue.map{errorMake(center: $0)}, encoding: .utf8)!.alongNext(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: OnReactiveCompatible = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = OnReactiveCompatible(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension HeterogenousViewController {
    /// 返回按钮事件
    ...
    private func popAnimated(animated _: Bool = true) {
    ...
    @objc func clickTarget() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kLet_scaleName - 2 * 26, height: kLet_environmentText - 46 - 127 - kLet_partyKeyTitle)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kLet_scaleName / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kLet_environmentText, width: kLet_scaleName, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = MechanicallyThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kLet_maleName)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - kLet_partyKeyTitle)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: TitleSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func positionPathEffect() {
        ...
        if self.videoEditHandler != nil {
            ...
            KeyInsertListener.shared.empty(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.popAnimated()
    ...
    @objc func courseOfActionDataSender(sender: UIButton) {
...
extension HeterogenousViewController {
    ...
    private func setting() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(finished), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.pullRange(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func leadership() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + kLet_partyKeyTitle)
        }
