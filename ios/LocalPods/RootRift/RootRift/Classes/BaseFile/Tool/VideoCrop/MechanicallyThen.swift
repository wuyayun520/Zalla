...
fileprivate let str_kitName:[UInt8] = [0x62,0x65,0x62,0x7f,0x23,0x68,0x64,0x6f,0x6e,0x79,0x31,0x22,0x2b,0x63,0x6a,0x78,0x2b,0x65,0x64,0x7f,0x2b,0x69,0x6e,0x6e,0x65,0x2b,0x62,0x66,0x7b,0x67,0x6e,0x66,0x6e,0x65,0x7f,0x6e,0x6f]
...
typealias SlidingHandler = (_ direction: TitleSlideDirection) -> Void
...
class MechanicallyThen: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (kLet_licenseFormatData - self.contentView.contentOffset.x)
    ...
    init(frame: CGRect, url: URL, imgWidth: CGFloat, maxDuration: CGFloat, minVideoDuration: CGFloat) {
        ...
        //: setupSubviews()
        mediumMagnitudeimate()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_kitName.map{$0^11}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.imagePoint(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: ConstraintCropOverlay = {
        ...
        let overlay = ConstraintCropOverlay(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension MechanicallyThen {
    ...
    private func imagePoint(point: CGFloat) -> UIImage {
...
extension MechanicallyThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_licenseFormatData - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.voiceName(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.voiceName(isShow: true)
    }
...
extension MechanicallyThen {
    ...
    private func mediumMagnitudeimate() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_licenseFormatData)
