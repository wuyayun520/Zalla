...
protocol SlideNavigationDelegate: AnyObject {
...
class StreamViewController: UIViewController {
    ...
    weak var delegate: SlideNavigationDelegate?
    ...
    override func viewDidLoad() {
        ...
        if selectedBtn != nil {
            ...
            quantitativeRelationClick(selectedBtn!)
        }
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = ListMacroDefine.getScreenHeight() - (topDistance + kTitle_H)
        var height = ListMacroDefine.mark() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= kLet_descriptionValue
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: ListMacroDefine.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: ListMacroDefine.width(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(ListMacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(ListMacroDefine.width()), height: 0)
        ...
        scrollView.semanticContentAttribute = FactoryShowThen.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: ListMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: ListMacroDefine.width()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: ListMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: ListMacroDefine.width()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = FactoryShowThen.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(quantitativeRelationClick), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = ListMacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = ListMacroDefine.width() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(ListMacroDefine.getScreenWidth()-originX-titleWidth)
            let x = FactoryShowThen.shared.direction == .leftToRight ? originX:(ListMacroDefine.width()-originX-titleWidth)
...
extension StreamViewController {
    ...
    @objc private func quantitativeRelationClick(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        equalTap(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * ListMacroDefine.getScreenWidth()
        let offset_x = CGFloat(index) * ListMacroDefine.width()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        appStream(withIndex: index)
        //: seletedViewController(withIndex: index)
        viewIndex(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func appStream(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * ListMacroDefine.getScreenWidth()
        let origin_x = CGFloat(index) * ListMacroDefine.width()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: ListMacroDefine.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: ListMacroDefine.width(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func equalTap(_ button: UIButton) {
    ...
    func windowCenter(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= ListMacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= ListMacroDefine.width() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - ListMacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - ListMacroDefine.width() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - ListMacroDefine.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - ListMacroDefine.width()
...
extension StreamViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / ListMacroDefine.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / ListMacroDefine.width())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            equalTap(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        appStream(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / ListMacroDefine.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / ListMacroDefine.width())
        ...
        //: let scaleR = scrollView.contentOffset.x / ListMacroDefine.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / ListMacroDefine.width() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
