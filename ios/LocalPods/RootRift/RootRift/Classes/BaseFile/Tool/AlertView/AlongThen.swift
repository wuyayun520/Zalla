...
fileprivate let str_liveValue:[Character] = ["b","t","n","_","s","h","o","w","_","a","l","e","r","t","B","a","c","k","g","r","o","u"]
fileprivate let str_countTitle:String = "ND"

/*: "btn_alertBackground_false" :*/
fileprivate let str_renderName:[Character] = ["b","t","n","_","a","l","e","r","t","B","a","c","k","g","r","o","u"]
fileprivate let str_modelData:String = "nd_make return to input self"
fileprivate let str_colorTitle:String = "FALSE"
...
public class AlongThen: NSObject {
    /// 通用回调
...
public extension AlongThen {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: ReloadAlertConfig) -> Void
    ...
    internal class func harvest(title: String? = nil,
                                //: message: String?  = nil,
                                message: String? = nil,
                                ...
                                config: ReloadAlertConfig? = nil)
    {
        //: customAlert(title: title,
        jumpTime(title: title,
                 //: message: message,
                 message: message,
    ...
    @objc internal class func swageBlock(title: String? = nil,
    ...
    class func jumpTime(title: String? = nil,
                        ...
                        config: ReloadAlertConfig? = nil)
    {
        ...
        let model = config ?? ReloadAlertConfig()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = DateWriteBirdSEyeViewReactiveCompatible(title: title,
        ...
        //: ListMacroDefine.getWindow().addSubview(alertView)
        ListMacroDefine.startLoad().addSubview(alertView)
    ...
    class func teachingMethod() {
        //: ListMacroDefine.getWindow().subviews.forEach { (view) in
        ListMacroDefine.startLoad().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: DateWriteBirdSEyeViewReactiveCompatible.self) {
    ...
    class func atomicTotal85(title: String, isTouch: Bool) {
        //: ListMacroDefine.getWindow().subviews.forEach { (view) in
        ListMacroDefine.startLoad().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: DateWriteBirdSEyeViewReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: DateWriteBirdSEyeViewReactiveCompatible = view as! DateWriteBirdSEyeViewReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.itemTouch(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class ReloadAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: ListMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .conversationSize(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.ending()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .conversationSize(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.ending()
    ...
    public var buttonFont: UIFont = .conversationSize(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.smallFor()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(str_liveValue) + str_countTitle.lowercased())

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(str_renderName) + String(str_modelData.prefix(3)) + str_colorTitle.lowercased())
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .conversationSize(type: .Medium, fontSize: 16)
