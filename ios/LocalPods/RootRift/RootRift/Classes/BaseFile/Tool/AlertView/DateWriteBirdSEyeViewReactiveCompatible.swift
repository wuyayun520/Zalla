...
fileprivate let str_pathEffectValue:[Character] = ["b","t","n","_","m","e","_","b","a","c","k","_","c","o","n"]
fileprivate let str_endTitle:[Character] = ["t","i","n","u","e"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_emptyValue:[UInt8] = [0x1a,0x1d,0x1a,0x7,0x5b,0x10,0x1c,0x17,0x16,0x1,0x49,0x5a,0x53,0x1b,0x12,0x0,0x53,0x1d,0x1c,0x7,0x53,0x11,0x16,0x16,0x1d,0x53,0x1a,0x1e,0x3,0x1f,0x16,0x1e,0x16,0x1d,0x7,0x16,0x17]

private func failWrite(label num: UInt8) -> UInt8 {
    return num ^ 115
}
...
// __CLOSE_PRINT__
//
//  DateWriteBirdSEyeViewReactiveCompatible.swift
//  AbroadTalking
//
...
class DateWriteBirdSEyeViewReactiveCompatible: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: ReloadAlertConfig
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.smallFor().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(blockView), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setTitleColor(alertConfig.rightColor, for: .normal)
        //: let image = UIImage.withView(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
        let image = UIImage.withView(name: alertConfig.rightBackgroundImage).withTintColor(.smallFor())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(videoFinish), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: ReloadAlertConfig)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(str_pathEffectValue) + String(str_endTitle))
        }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_emptyValue.map{failWrite(label: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func itemTouch(title: String, isTouch: Bool) {
        ...
        //: if !isTouch {
        if !isTouch {
            //: rightBtn.setBackgroundImage(UIImage.withView(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            rightBtn.setBackgroundImage(UIImage.withView(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.withView(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
