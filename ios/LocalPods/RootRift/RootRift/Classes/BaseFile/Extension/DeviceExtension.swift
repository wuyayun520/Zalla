...
/*: "RSA加密失败，str:  :*/
fileprivate let str_viewValue:[Character] = ["R","S","A","加","密","失","\u{8d25}","，","s","t","r",":"," "]

/*: ." :*/
fileprivate let str_replyMenuName:[Character] = ["."]

/*: "weixin://" :*/
fileprivate let str_nearData:String = "wrapeixin"
fileprivate let str_femaleData:[Character] = [":","/","/"]

/*: "dingtalk://" :*/
fileprivate let str_sourceOfText:[Character] = ["d","i","n","g","t","a","l","k",":","/","/"]

/*: "lark://" :*/
fileprivate let str_byData:String = "lab return makelark://"

/*: "wxwork://" :*/
fileprivate let str_modeTitle:String = "wxwork://key let of extra"

/*: "p1" :*/
fileprivate let str_actionText:[UInt8] = [0x1d,0x5c]

private func colorGesture(shared num: UInt8) -> UInt8 {
    return num ^ 109
}

/*: "p2" :*/
fileprivate let str_insideName:[UInt8] = [0xa0,0xe2]

private func leadingRequest(cell num: UInt8) -> UInt8 {
    return num ^ 208
}

/*: "p3" :*/
fileprivate let str_valueTitle:[UInt8] = [0x28,0x6b]

private func meanSun(text num: UInt8) -> UInt8 {
    return num ^ 88
}

/*: "p4" :*/
fileprivate let str_rangeValue:[UInt8] = [0xe5,0xa1]

private func eachCenter(temp num: UInt8) -> UInt8 {
    return num ^ 149
}

/*: "p5" :*/
fileprivate let str_requestName:[UInt8] = [0x78,0x3d]

private func oldEqual(text num: UInt8) -> UInt8 {
    return num ^ 8
}

/*: "p7" :*/
fileprivate let str_addName:[UInt8] = [0x37,0x70]

/*: "deviceId" :*/
fileprivate let str_kickContent:[UInt8] = [0x21,0x20,0x33,0x2c,0x26,0x20,0xc,0x21]

/*: "p0" :*/
fileprivate let str_makeValue:[UInt8] = [0x30,0x70]

/*: "weixin" :*/
fileprivate let str_needName:String = "startixin"

/*: "wxwork" :*/
fileprivate let str_addValue:[Character] = ["w","x","w","o","r","k"]

/*: "dingtalk" :*/
fileprivate let str_colorValue:[Character] = ["d","i","n","g","t","a","l","k"]

/*: "lark" :*/
fileprivate let str_iconValue:String = "lrangerk"
...
extension UIDevice {
    ...
    class func blowerCloseDominoGetDeviceIdentifier(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.modelSub())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func secretScreen() -> ([String]) {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func flickMini(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(str_viewValue)) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (str_nearData.replacingOccurrences(of: "wrap", with: "w") + String(str_femaleData))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (String(str_sourceOfText))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(str_byData.suffix(7)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(str_modeTitle.prefix(9)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(kLet_topText) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: kLet_topText)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = GazetteThen.makeConstraint() || GazetteThen.frameGesture()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = flickMini(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: str_kickContent.map{$0^69}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((str_needName.replacingOccurrences(of: "start", with: "we")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(str_addValue)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(str_colorValue)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((str_iconValue.replacingOccurrences(of: "range", with: "a")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
