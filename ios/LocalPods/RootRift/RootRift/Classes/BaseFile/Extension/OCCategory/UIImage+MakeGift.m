...
- (UIImage *)errorTo {
    ...
    return [self mentalPicture:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)timeline {
    //: return [self wcCompres:NO];
    return [self mentalPicture:NO];
}
...
- (UIImage *)mentalPicture:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self fileSize:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self imageLocal:size];
...
- (CGSize)fileSize:(Boolean)isSession {
...
+ (NSData *)qualifyVideoSumSize:(UIImage *)image mentalPicture:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG price:scalesize effect:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG file:(fileSize*1024)];
    }
...
- (UIImage*)toCurrent:(CGSize)targetSize{
...
- (UIImage *)price:(CGSize)targetSize effect:(BOOL)isScale{
...
- (NSData *)file:(NSInteger)fileSize{
...
- (UIImage *)imageLocal:(CGSize)newSize {
