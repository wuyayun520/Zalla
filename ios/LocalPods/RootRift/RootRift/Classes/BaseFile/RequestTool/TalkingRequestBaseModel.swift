...
/*: "http://stat. :*/
fileprivate let str_allValue:[Character] = ["h","t"]
fileprivate let str_labelText:String = "tp:/if sex extension equal conversation"

/*: .com" :*/
fileprivate let str_arrayName:String = "tip equal delay.com"

/*: "http://" :*/
fileprivate let str_fromName:String = "model"
fileprivate let str_blockData:String = "let container colorttp://"

/*: "https://" :*/
fileprivate let str_targetCollectionData:String = "sourcet"
fileprivate let str_titleActionName:String = "text path cut maketps://"

/*: "http://m. :*/
fileprivate let str_viewCommonElseName:String = "cell cell viewhttp:"
fileprivate let str_makeTitle:String = "top image present as user//m."

/*: "http:// :*/
fileprivate let str_addData:[Character] = ["h","t","t","p",":","/","/"]

/*: "-1" :*/
fileprivate let str_topTitle:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let str_positionManagerText:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let str_cardMakeData:[Character] = ["1","9","9","0","-"]
fileprivate let str_engineData:[Character] = ["0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let str_gestureValue:[Character] = ["M","M","-","d","d"]
fileprivate let str_storageText:String = "post progress mm live level-yyyy"

/*: "id" :*/
fileprivate let str_showValue:String = "deviced"
...
class IndexRequestModel: NSObject {
    ...
    var requestType: BottomRequestType?
    ...
    override init() {
        ...
        self.requestServer = GazetteThen.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func dataObject() -> Bool {
...
struct ResponseModelType: HandyJSON {
    //: var errno: Int!
...
public struct ScaleErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct RawModelType: HandyJSON {
    ...
    var CSConfig = WhiteConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(str_allValue) + String(str_labelText.prefix(4)) + "/stat.") + "\(kLet_barRecordScreenText)" + (String(str_arrayName.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((str_fromName.replacingOccurrences(of: "model", with: "h") + String(str_blockData.suffix(6)))) && !statDomain.hasPrefix((str_targetCollectionData.replacingOccurrences(of: "source", with: "h") + String(str_titleActionName.suffix(6)))) {
                //: return "http://".appending(statDomain)
                return (str_fromName.replacingOccurrences(of: "model", with: "h") + String(str_blockData.suffix(6))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(str_viewCommonElseName.suffix(5)) + String(str_makeTitle.suffix(4))) + "\(kLet_barRecordScreenText)" + (String(str_arrayName.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(str_addData)) + "\(urlDomain)"
        }
    }
...
public class EqualMeasurable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [AtFailHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [AtFailHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [AtFailHandyJSON] = [] // 语音收费设置
    ...
    var tabPageBanner = [CurrentModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = BecomeHandyJSON() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class TableMeasurable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [ShowHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [ShowHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [DirectTransformable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (str_positionManagerText.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.contextCord(self.birthday ?? (String(str_cardMakeData) + String(str_engineData)), dateFormat: (String(str_gestureValue) + String(str_storageText.suffix(5))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.toFilter().year else { return "0" }
...
struct DirectTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct ShowHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func compareEqualsTo(compareTo: ShowHandyJSON) -> Bool {
        //: return
        return
...
struct ResultInfoModel {
    ...
    init() {
        ...
        self.nickName = ""
        //: setBirth()
        bagCommencementChange()
    }

    //: mutating func setBirth() {
    mutating func bagCommencementChange() {
...
struct EndUserMeasurable: HandyJSON {
    ...
    var aboutMe: [QuantityerchangeHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [QuantityerchangeHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct QuantityerchangeHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [ShowHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct WhiteConfigModel: HandyJSON {
...
struct BecomeHandyJSON: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
