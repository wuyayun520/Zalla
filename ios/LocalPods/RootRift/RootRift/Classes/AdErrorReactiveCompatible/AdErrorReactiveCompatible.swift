...
/*: "&sr=6" :*/
fileprivate let str_hiddenTitle:[Character] = ["&","s","r","=","6"]
...
// __CLOSE_PRINT__
//
//  AdErrorReactiveCompatible.swift
//  AbroadTalking
//
...
enum MethodValueConvertible: String, CaseIterable {
...
enum VoicePublishNameConvertible: String {
...
class AdErrorReactiveCompatible: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func viewType(urlType: VoicePublishNameConvertible) -> String {
        //: var url = WatercourseThen.share.appConfigMode.urlH5Domain
        var url = WatercourseThen.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(WatercourseThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : WatercourseThen.share.appUserConfigMode.fullRechargePage)
            url.append(WatercourseThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : WatercourseThen.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(WatercourseThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : WatercourseThen.share.appUserConfigMode.halfRechargePage)
            url.append(WatercourseThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : WatercourseThen.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = WatercourseThen.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = WatercourseThen.share.appUserConfigMode.halfRechargePage.appending((String(str_hiddenTitle)))
            //: url.append(WatercourseThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(WatercourseThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
