...
// __CLOSE_PRINT__
//
//  UnbeknownstRecognizerDelegate.swift
//  AbroadTalking
//
...
class UnbeknownstRecognizerDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: MediaCustomReflectable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.age()
            }
        }
    ...
    func golfBag() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.beforeFillEvent(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.modelCard()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.ending(), NSAttributedString.Key.font: UIFont.conversationSize(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.beforeFillEvent(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.ending(), NSAttributedString.Key.font: UIFont.conversationSize(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.ending()
...
extension UnbeknownstRecognizerDelegate: UINavigationControllerDelegate {
...
extension UnbeknownstRecognizerDelegate: UIGestureRecognizerDelegate {
