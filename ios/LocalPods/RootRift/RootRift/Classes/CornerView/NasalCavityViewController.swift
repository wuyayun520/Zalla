...
fileprivate let str_onSexByValue:[UInt8] = [0xf1,0xf6,0xf1,0xfc,0xb0,0xeb,0xf7,0xec,0xed,0xfa,0xc2,0xb1,0xa8,0xf0,0xe9,0xfb,0xa8,0xf6,0xf7,0xfc,0xa8,0xea,0xed,0xed,0xf6,0xa8,0xf1,0xf5,0xf8,0xf4,0xed,0xf5,0xed,0xf6,0xfc,0xed,0xec]

fileprivate func modelPath(signature num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  NasalCavityViewController.swift
//  AbroadTalking
//
...
class NasalCavityViewController: MusicViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: UncolouredTransitioningDelegate!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = UncolouredTransitioningDelegate(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_onSexByValue.map{modelPath(signature: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class UncolouredTransitioningDelegate: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kLet_scaleName, height: kLet_environmentText * 0.6)
    ...
    func addPlace(corners: UIRectCorner, radius: CGFloat) {
...
extension UncolouredTransitioningDelegate: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = AreaRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension UncolouredTransitioningDelegate: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            afterDown(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            willData(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func afterDown(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func willData(transitionContext: UIViewControllerContextTransitioning) {
...
class AreaRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: UncolouredTransitioningDelegate!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: UncolouredTransitioningDelegate, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func lesser() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(vanguardWith))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kLet_scaleName, height: kLet_environmentText)
