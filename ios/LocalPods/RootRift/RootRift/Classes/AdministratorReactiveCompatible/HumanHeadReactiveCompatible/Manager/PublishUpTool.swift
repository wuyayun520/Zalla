...
class PublishUpTool: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: PublishChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = BottomChatConfig()          // 视频渲染配置类
    ...
    private var trtcView: DataStorageMediumView?    // 预览view（使用商汤美颜）
...
extension PublishUpTool {
    ...
    func actionChange(_ frontCamera: Bool) {
        ...
        self.buttonRow(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func cameraDevote(_ openCamera: Bool) {
        ...
        self.too(isOpen: openCamera)
    }
    
    /// 更改视频采集渲染展示位置
    //: func changeRenderView() {
    func mistranslateView() {
        ...
        DispatchQueue.main.async {
            ...
            self.somebody(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension PublishUpTool {
    ...
    private func buttonRow(front: Bool) {
        ...
        self.trtcView?.camer(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func too(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_errorDeviceAppValue == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.toQuantityUser(isOpen: isOpen)
...
extension PublishUpTool {
    ...
    func quantityerrupt() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(totalrudeOn(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(reportEqual(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(magnitudeerrupted(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(imageAddNotif(notif:)),
    ...
    @objc private func totalrudeOn(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func reportEqual(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func magnitudeerrupted(notif: Notification) {
    ...
    @objc func imageAddNotif(notif: Notification) {
...
extension PublishUpTool {
    ...
    func somebody(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_errorDeviceAppValue == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = DataStorageMediumView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.checkOutSub(isSmallWindow: smallWindow)
        //: if smallWindow { 
