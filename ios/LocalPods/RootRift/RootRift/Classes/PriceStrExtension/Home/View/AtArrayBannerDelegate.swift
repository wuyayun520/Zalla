...
fileprivate let str_pathText:[UInt8] = [0xe4,0xe3,0xe4,0xf9,0xa5,0xee,0xe2,0xe9,0xe8,0xff,0xb7,0xa4,0xad,0xe5,0xec,0xfe,0xad,0xe3,0xe2,0xf9,0xad,0xef,0xe8,0xe8,0xe3,0xad,0xe4,0xe0,0xfd,0xe1,0xe8,0xe0,0xe8,0xe3,0xf9,0xe8,0xe9]

private func messageFill(me num: UInt8) -> UInt8 {
    return num ^ 141
}

/*: "JXBannerCellID" :*/
fileprivate let str_valueVideoTitle:String = "key info if color viewJXBanne"
fileprivate let str_officialData:[Character] = ["r","C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let str_showName:String = "equal next else view want#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let str_allowValue:String = "#BDB8B5self new return"
...
protocol PublishTitleViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class AtArrayBannerDelegate: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: PublishTitleViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [PassageListModel]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [CurrentModelType]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_pathText.map{messageFill(me: $0)}, encoding: .utf8)!)
    }
...
extension AtArrayBannerDelegate {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func barReceive() -> CGFloat {
    ...
    func sizeData(data: [PassageListModel]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func clearData(data: [CurrentModelType]) {
...
extension AtArrayBannerDelegate: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(str_valueVideoTitle.suffix(7)) + String(str_officialData)))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(str_showName.suffix(7))))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(str_allowValue.prefix(7))))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.paintTheLilyCell(model: model)
    }
}
...
extension AtArrayBannerDelegate {
    ...
    private func occurrent() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: kLet_scaleName - 30, height: 84))
        }
    }
}
