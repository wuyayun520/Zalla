...
fileprivate let str_playerValue:[UInt8] = [0xd0,0xd7,0xd0,0xcd,0x91,0xda,0xd6,0xdd,0xdc,0xcb,0x83,0x90,0x99,0xd1,0xd8,0xca,0x99,0xd7,0xd6,0xcd,0x99,0xdb,0xdc,0xdc,0xd7,0x99,0xd0,0xd4,0xc9,0xd5,0xdc,0xd4,0xdc,0xd7,0xcd,0xdc,0xdd]

/*: "Welcome to my room" :*/
fileprivate let str_onContent:String = "Welclength color user player"
fileprivate let str_modelMakeText:String = "image log string to modelto my "
fileprivate let str_valueText:[Character] = ["r","o","o","m"]

/*: "%@'s room" :*/
fileprivate let str_colorTabData:String = "%@\'s room"

/*: "CCCCCC" :*/
fileprivate let str_actualName:String = "cccccc"

/*: "#E1D0FC" :*/
fileprivate let str_sectionStretchReasonData:String = "label"
fileprivate let str_managerData:String = "E1DmainC"

/*: "#FFE3E5" :*/
fileprivate let str_addName:String = "#FFE3E5index make view make self"

/*: "get json error" :*/
fileprivate let str_selectedText:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r"]
fileprivate let str_acceptValue:[Character] = ["r","o","r"]
...
// __CLOSE_PRINT__
//
//  WhiteTitleReactiveCompatible.swift
//  RootRift
//
...
class WhiteTitleReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_playerValue.map{$0^185}, encoding: .utf8)!)
    }
    ...
    var listModel: ResultRequestListModel? {
        //: didSet {
        didSet {
            //: guard let listModel else { return }
            guard let listModel else { return }

            //: if String(listModel.uid) == WatercourseThen.share.loginUid {
            if String(listModel.uid) == WatercourseThen.share.loginUid {
                ...
                countryLab.text = (String(str_onContent.prefix(4)) + "ome " + String(str_modelMakeText.suffix(6)) + String(str_valueText)).localized
                //: countryLab.textColor = .appValueColor()
                countryLab.textColor = .obtrudeUpon()
                //: countryLab.font = UIFont.pingfangFont(type: .Medium, fontSize: 15)
                countryLab.font = UIFont.conversationSize(type: .Medium, fontSize: 15)
                ...
                countryLab.text = "\(listModel.country)"
                //: countryLab.textColor = .touchColor()
                countryLab.textColor = .touchColor()
                //: countryLab.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
                countryLab.font = UIFont.conversationSize(type: .Regular, fontSize: 15)
                //: countryLab.snp.updateConstraints { make in
                countryLab.snp.updateConstraints { make in
            ...
            headPicImgV.ofData(urlStr: listModel.pic)
            //: nameLab.text = "%@'s room".localizedArguments(listModel.name)
            nameLab.text = (str_colorTabData).alongNext(listModel.name)
            //: countryImgView.setUrlImage(urlStr: listModel.flag)
            countryImgView.ofData(urlStr: listModel.flag)
            //: hotLab.text = "\(listModel.score)"
            hotLab.text = "\(listModel.score)"

            //: let hotWidth  = hotLab.sizeThatFits(CGSize(width: ScreenWidth, height: 20)).width
            let hotWidth = hotLab.sizeThatFits(CGSize(width: kLet_scaleName, height: 20)).width
    ...
    private lazy var bgView: UIView = {
        ...
        v.layer.shadowColor = UIColor(hex: (str_actualName.uppercased()))?.withAlphaComponent(0.5).cgColor
    ...
    private lazy var bgvlayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: kLet_scaleName - 24, height: 76)
        //: layer.startPoint = CGPoint(x: 0, y: 0)
        layer.startPoint = CGPoint(x: 0, y: 0)
        //: layer.endPoint = CGPoint(x: 1, y: 0.5)
        layer.endPoint = CGPoint(x: 1, y: 0.5)
        //: layer.colors = [UIColor(hex: "#E1D0FC")?.cgColor, UIColor(hex: "#FFE3E5")?.cgColor]
        layer.colors = [UIColor(hex: (str_sectionStretchReasonData.replacingOccurrences(of: "label", with: "#") + str_managerData.replacingOccurrences(of: "main", with: "0F")))?.cgColor, UIColor(hex: (String(str_addName.prefix(7))))?.cgColor]
    ...
    private lazy var liveSvga: SVGAPlayer = {
        ...
        do {
            ...
            let url = StreamReactiveCompatible.default.picName(type: .Party_List)
            ...
            printLog(message: (String(str_selectedText) + String(str_acceptValue)))
        }
    ...
    private lazy var hotLab: UILabel = {
        ...
        lab.font = UIFont.conversationSize(type: .Medium, fontSize: 13)
...
extension WhiteTitleReactiveCompatible {
    // 添加视图
    //: private func setupSubviews() {
    private func toolSubviews() {
