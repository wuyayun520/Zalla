...
/*: "img_home_shadow" :*/
fileprivate let str_pageData:[Character] = ["i","m","g","_","h","o","m","e","_","s","h","a","d","o"]
fileprivate let str_layerName:[Character] = ["w"]

/*: "#9610FF" :*/
fileprivate let str_colorData:String = "#9610Fin for count play center"
fileprivate let str_replyTextContent:String = "close"

/*: "#8566FF" :*/
fileprivate let str_keyValue:[Character] = ["#","8","5","6","6"]
fileprivate let str_liveName:String = "makemake"

/*: "Random Video" :*/
fileprivate let str_titleData:[Character] = ["R","a","n"]
fileprivate let str_hiddenTitle:[Character] = ["d","o","m"," ","V","i","d","e","o"]

/*: "icon_match_call_free" :*/
fileprivate let str_panName:[Character] = ["i","c","o","n","_","m"]
fileprivate let str_miniData:String = "view selfatch_"
fileprivate let str_headData:String = "value to manager_free"

/*: "Free" :*/
fileprivate let str_intimateInputText:String = "if error view line sizeFree"

/*: "Scroll_upto_discove" :*/
fileprivate let str_arrayValue:String = "in request upper pathScrol"
fileprivate let str_addLastViewTitle:[Character] = ["t","o","_","d","i","s","c","o","v","e"]

/*: "icon_VectorStroke" :*/
fileprivate let str_subName:[Character] = ["i","c","o","n","_","V","e","c","t","o","r","S"]
fileprivate let str_valueData:String = "homeoke"

/*: "get json error" :*/
fileprivate let str_picData:String = "gpath"
fileprivate let str_rowTopValue:String = "false path to manager maket js"
...
protocol OnHeadObjectProtocol: NSObjectProtocol {
    ///
    //: func numberOfCountInCardView(cardView: TalkingClubCardView) -> Int
    func partyBegin(cardView: PolytonalismLogView) -> Int
    //: func cardViewCellForRowAtIndex(cardView: TalkingClubCardView, index: Int) -> TalkingClubCardViewCell
    func gestureCon(cardView: PolytonalismLogView, index: Int) -> TalkingNetworkCompartmentView
    //: func CellForRowAtIndexModel(index: Int) -> TalkingMatchResultModel
    func freebieIndex(index: Int) -> AchromaticColorMeasurable
}

//: @objc protocol TalkingClubCardViewDelegate: NSObjectProtocol {
@objc protocol OrientationViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func didRemoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func withOrientation(cardView: PolytonalismLogView, cardViewCell: TalkingNetworkCompartmentView, forRowAtIndex: Int)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func colorAdd(cardViewCell: TalkingNetworkCompartmentView, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func spend(cardViewCell: TalkingNetworkCompartmentView, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func punchedCardOn(cardView: PolytonalismLogView, cardViewCell: TalkingNetworkCompartmentView, forMovePoint: CGPoint)
    /// 随机匹配视频
    //: @objc optional func clubCardRandowBtnClick()
    @objc optional func halogenClick()
}

//: class TalkingClubCardView: UIView {
class PolytonalismLogView: UIView {
    ...
    var maxRemoveDistance = kLet_scaleName / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [TalkingNetworkCompartmentView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: OnHeadObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: OrientationViewDelegate?
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.withView(name: "img_home_shadow")
        imag.image = UIImage.withView(name: (String(str_pageData) + String(str_layerName)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.distancePlay(colors: [UIColor(hex: (String(str_colorData.prefix(6)) + str_replyTextContent.replacingOccurrences(of: "close", with: "F")))!.cgColor, UIColor(hex: (String(str_keyValue) + str_liveName.replacingOccurrences(of: "make", with: "F")))!.cgColor], size: CGSize(width: kLet_scaleName - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(str_titleData) + String(str_hiddenTitle)).localized, for: .normal)
    ...
    lazy var freeImag: UIButton = {
        //: let img = UIButton()
        let img = UIButton()
        //: img.setBackgroundImage(UIImage.withView(name: "icon_match_call_free"), for: .normal)
        img.setBackgroundImage(UIImage.withView(name: (String(str_panName) + String(str_miniData.suffix(5)) + "call" + String(str_headData.suffix(5)))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(str_intimateInputText.suffix(4))).localized, for: .normal)
...
extension PolytonalismLogView {
    ...
    private func showDoing() {
        ...
        image.contentMode = .scaleAspectFill
        //: image.image = UIImage.withView(name: "Scroll_upto_discove")
        image.image = UIImage.withView(name: (String(str_arrayValue.suffix(5)) + "l_up" + String(str_addLastViewTitle)))
        ...
        image2.contentMode = .scaleAspectFill
        //: image2.image = UIImage.withView(name: "icon_VectorStroke")
        image2.image = UIImage.withView(name: (String(str_subName) + str_valueData.replacingOccurrences(of: "home", with: "tr")))
        ...
        mentioner(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func mentioner(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = StreamReactiveCompatible.default.picName(type: .Club_guidePage)
            ...
            printLog(message: (str_picData.replacingOccurrences(of: "path", with: "e") + String(str_rowTopValue.suffix(4)) + "on error"))
        }
    }
    ...
    @objc private func moveClick() {
        ...
        self.delegate?.halogenClick?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func collectionReloadAnimated(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.partyBegin(cardView: self) ?? 0
    ...
    func playerLoad(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.gestureCon(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? TalkingNetworkCompartmentView(), at: 0)
    ...
    func clickAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: TalkingNetworkCompartmentView = self.visibleCells[i] as! TalkingNetworkCompartmentView
            ...
            if isVisbleLast {
                ...
                self.delegate?.spend?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                transformLoad(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func transformLoad(cell: TalkingNetworkCompartmentView, transform: CGAffineTransform) {
    ...
    func scaleOfMeasurementInde(inde: Int) -> Int {
    ...
    func sinceUser(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func talkBy(identifier: String) -> TalkingNetworkCompartmentView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! MetalworksView.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: MetalworksView = cellclass.init(reuseIdentifier: identifier)
    ...
    func contemporary(index: Int) -> TalkingNetworkCompartmentView {
        //: let visibleIndex = visibleIndex(inde: index)
        let visibleIndex = scaleOfMeasurementInde(inde: index)
        //: let cell = self.visibleCells[visibleIndex]
        let cell = self.visibleCells[visibleIndex]
        //: return cell as! TalkingClubCardViewCell
        return cell as! TalkingNetworkCompartmentView
    }

    /// 获取cell对应的index
    //: func indexForCell(cell: TalkingClubCardViewCell) -> Int {
    func forthright(cell: TalkingNetworkCompartmentView) -> Int {
        //: let arr = self.visibleCells as? [TalkingClubCardViewCell]
        let arr = self.visibleCells as? [TalkingNetworkCompartmentView]
    ...
    func dismissFrom(direction: NoseSwipeDirection) {
        //: if self.visibleCells.count == 0 { return }
        if self.visibleCells.count == 0 { return }
        //: let topcell = self.visibleCells.last as? TalkingClubCardViewCell
        let topcell = self.visibleCells.last as? TalkingNetworkCompartmentView
        //: topcell?.removeFromSuperviewSwipe(direction: direction)
        topcell?.artifact(direction: direction)
    }
}

/// PassageObjectProtocol
//: extension TalkingClubCardView: TalkingClubCardViewCellDelegete {
extension PolytonalismLogView: PassageObjectProtocol {
    ...
    func rankCell(cell: TalkingNetworkCompartmentView) {
        ...
        self.delegate?.withOrientation?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.partyBegin(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.colorAdd?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            playerLoad(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func playingCardForAttractor(cell: TalkingNetworkCompartmentView, forMovePoint: CGPoint) {
        ...
        self.delegate?.punchedCardOn?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
...
extension PolytonalismLogView {
    //: func reloadData() {
    func outsideEqualScreen() {
        //: reloadDataAnimated(anim: false)
        collectionReloadAnimated(anim: false)
    }

    //: func hidePriceBtn() {
    func dataTotaleraction() {
    ...
    func storage() {
        ...
        let index = AlbumKeyRepresentable.randow
