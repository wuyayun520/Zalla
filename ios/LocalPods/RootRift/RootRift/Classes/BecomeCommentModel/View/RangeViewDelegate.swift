...
fileprivate let str_postViewText:[UInt8] = [0x82,0x85,0x82,0x9f,0xc3,0x88,0x84,0x8f,0x8e,0x99,0xd1,0xc2,0xcb,0x83,0x8a,0x98,0xcb,0x85,0x84,0x9f,0xcb,0x89,0x8e,0x8e,0x85,0xcb,0x82,0x86,0x9b,0x87,0x8e,0x86,0x8e,0x85,0x9f,0x8e,0x8f]

private func sourceContent(video num: UInt8) -> UInt8 {
    return num ^ 235
}
...
class RangeViewDelegate: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_postViewText.map{sourceContent(video: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func barUi() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(toVia))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(moveObject))
...
extension RangeViewDelegate: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func toVia(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func moveObject(tap: UIGestureRecognizer) {
    ...
    func root(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.ofData(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func dismissImage(image: UIImage) {
