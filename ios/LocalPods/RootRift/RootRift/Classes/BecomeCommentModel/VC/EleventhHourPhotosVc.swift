...
fileprivate let str_messageValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "RangeViewDelegate" :*/
fileprivate let str_viewValue:[Character] = ["T","a","l","k","i","n","g","M","o","m","e","n","t","P","h","o","t","o"]
fileprivate let str_bottomRawText:String = "sPrevirandom select view kind last"

/*: "Save successfully" :*/
fileprivate let str_willName:[Character] = ["S","a","v","e"," ","s","u"]
fileprivate let str_dataEachValue:String = "ccclicks"

/*: / :*/
fileprivate let str_arrayValue:[Character] = ["/"]
...
class EleventhHourPhotosVc: SideEffectReactiveCompatible {
    ...
    init(momentModel: CellMomentModel, index: Int, type: AutomaticallyArrayPhotoType) {
        ...
        if FactoryShowThen.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(kLet_scaleName), y: 0)
    ...
    init(images: [UIImage], index: Int, type: AutomaticallyArrayPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_messageValue.reversed(), encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: kLet_scaleName, height: kLet_environmentText)
        ...
        view.frame = CGRect(x: 0, y: 0, width: kLet_scaleName, height: kLet_environmentText)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(kLet_scaleName), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(kLet_scaleName), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(kLet_scaleName), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: "TalkingMomentPhotosPreviewCell")
        view.register(RangeViewDelegate.self, forCellWithReuseIdentifier: (String(str_viewValue) + String(str_bottomRawText.prefix(6)) + "ewCell"))
...
//    lazy var downloadBtn: MediaControl = {
//        let btn = MediaControl.init()
...
extension EleventhHourPhotosVc {
...
//        ColorPermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: OnModelType = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.sizeExecuteMsg(showMsg: (String(str_willName) + str_dataEachValue.replacingOccurrences(of: "click", with: "es") + "fully").localized)
                }
                //: } else {
            } else {
...
extension EleventhHourPhotosVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func dataFormatting() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func quits() {
        //: if  currenModel.sex==WatercourseThen.share.loginUserMode.sex {
        if currenModel.sex == WatercourseThen.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: RangeViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: (String(str_viewValue) + String(str_bottomRawText.prefix(6)) + "ewCell"), for: indexPath) as! RangeViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: OnModelType = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.root(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.dismissImage(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.dismissImage(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: RangeViewDelegate = cell as! RangeViewDelegate
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / kLet_scaleName)
        //: if myType == .normal {
        if myType == .normal {
