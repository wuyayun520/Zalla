...
/*: "id" :*/
fileprivate let str_detailData:String = "framed"

/*: "%@ reply %@：%@" :*/
fileprivate let str_aspectValue:[Character] = ["%","@"," ","r","e","p"]
fileprivate let str_iconBlockTitle:String = "view array app returnly %@：%@"
...
// __CLOSE_PRINT__
//
//  CommercialMessageTransformable.swift
//  AbroadTalking
//
...
struct CommercialMessageTransformable: HandyJSON {
    //: var cid: String?
...
extension CommercialMessageTransformable {
    //: mutating func caculateReplyItemHeight() {
    mutating func isometric() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(str_aspectValue) + String(str_iconBlockTitle.suffix(8))), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(TableReactiveCompatible.lastPop(text: tempContent, size: CGSize(width: kLet_scaleName - 82, height: CGFloat(MAXFLOAT)), font: UIFont.conversationSize(type: .Regular, fontSize: 14), lineNumber: 0).height)
