...
/*: "id" :*/
fileprivate let str_numberTimePathTitle:[Character] = ["i","d"]
...
struct CellMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var videoInfo: AdAutomaticallyHandyJSON?
...
struct AdAutomaticallyHandyJSON: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: TakeAwayUTF8Initializable?
}

///
//: enum MomentVideoStatus: String {
enum TakeAwayUTF8Initializable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension CellMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func beyondRequest(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(TableReactiveCompatible.lastPop(text: content!, size: CGSize(width: kLet_scaleName - 54, height: CGFloat(MAXFLOAT)), font: UIFont.conversationSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ViewTransformable.Photo.rawValue || momentType == ViewTransformable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
