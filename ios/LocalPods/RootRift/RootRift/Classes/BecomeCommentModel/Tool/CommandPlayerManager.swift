...
/*: "%@%@" :*/
fileprivate let str_viewValue:String = "%make%make"

/*: "/TXCache" :*/
fileprivate let str_touchName:String = "text medium your/TXCache"
...
enum FactoryCommandPlayerStatus {
    //: case Unknown
    case Unknown // 未知
    //: case ReadyToPlay
...
protocol QualityReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func rangeDoing(player: CommandPlayerManager, status: FactoryCommandPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func durationTime(player: CommandPlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func digitiser(player: CommandPlayerManager, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class CommandPlayerManager: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: QualityReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: FactoryCommandPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = CommandPlayerManager()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(str_touchName.suffix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(toRun), userInfo: nil, repeats: true)
...
extension CommandPlayerManager {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func shouldUserAdd(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: kLet_localContent.showUponCompute(), with: kLet_barRecordScreenText)
    ...
    @objc func toRun() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.digitiser(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func nextApp(renderMode: NoseGiftRenderMode) {
    ...
    func backgroundTime(time: Float) {
    ...
    func enableGenerate(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if NameSocketManager.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func followPause() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.rangeDoing(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func passkeyView() -> Bool {
        //: return self.player.isPlaying()
        return self.player.isPlaying()
    }

    //: func removeVideoWidget() {
    func applicationWidget() {
        //: self.player.removeVideoWidget()
        self.player.removeVideoWidget()
    }
...
extension CommandPlayerManager: TXVodPlayListener {
