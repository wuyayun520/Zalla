...
fileprivate let str_objectName:[UInt8] = [0x5,0x2,0x5,0x18,0x44,0xf,0x3,0x8,0x9,0x1e,0x56,0x45,0x4c,0x4,0xd,0x1f,0x4c,0x2,0x3,0x18,0x4c,0xe,0x9,0x9,0x2,0x4c,0x5,0x1,0x1c,0x0,0x9,0x1,0x9,0x2,0x18,0x9,0x8]

private func viewSection(error num: UInt8) -> UInt8 {
    return num ^ 108
}

/*: "move" :*/
fileprivate let str_emptyRangeName:String = "originve"
...
class PublishScreenView: UIView {
    ...
    var danumuMsgArray: [NetworkScreenModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [FailLabelView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<NetworkScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_objectName.map{viewSection(error: $0)}, encoding: .utf8)!)
    }
    ...
    func betweenListReceive(_ danumuMsgModel: NetworkScreenModel?) {
    ...
    @objc func saveRecord(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(saveRecord(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension PublishScreenView: UIGestureRecognizerDelegate {
    ...
    func microscopic(for danumuMsgModel: NetworkScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = FailLabelView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_emptyRangeName.replacingOccurrences(of: "origin", with: "mo")))
    ...
    func modeltoPlace(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                microscopic(for: model, isFirst: isFirst)
...
extension PublishScreenView {
    // 添加视图
    //: private func setupSubviews() {
    private func setupRecordSubviews() {
