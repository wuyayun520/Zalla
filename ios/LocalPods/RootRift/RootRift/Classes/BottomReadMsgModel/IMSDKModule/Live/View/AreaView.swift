...
fileprivate let str_itemToValue:[UInt8] = [0x64,0x69,0x64,0x6f,0x23,0x5e,0x6a,0x5f,0x60,0x6d,0x35,0x24,0x1b,0x63,0x5c,0x6e,0x1b,0x69,0x6a,0x6f,0x1b,0x5d,0x60,0x60,0x69,0x1b,0x64,0x68,0x6b,0x67,0x60,0x68,0x60,0x69,0x6f,0x60,0x5f]

fileprivate func modelResult(day num: UInt8) -> UInt8 {
    let value = Int(num) + 5
    if value > 255 {
...
/*: "#F5F5F5" :*/
fileprivate let str_firstValue:String = "height var player#F5F5F5"

/*: "level_ :*/
fileprivate let str_assetName:[Character] = ["l","e","v","e","l","_"]
...
class AreaView: UITableViewCell {
    //: var currenModel = TalkingLiveRoomOnlineListModel()
    var currenModel = DateMeasurable()
    //: var index = 0
    var index = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_itemToValue.map{modelResult(day: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(str_firstValue.suffix(7))))
        //: return view
        return view
        //: }()
    }()
...
extension AreaView {
    ...
    func upFinish(model: DateMeasurable, index: Int) {
        //: currenModel = model
        currenModel = model
        //: self.index = index
        self.index = index
        ...
        IconImg.ofData(urlStr: model.headPic)
        //: nameLabel.text = model.nickname
        nameLabel.text = model.nickname
        //: levelIcon.image = UIImage.withView(name: "level_\(model.level )")
        levelIcon.image = UIImage.withView(name: (String(str_assetName)) + "\(model.level)")
    }
}
...
extension AreaView {
    ...
    private func viewsTextAppearance() {
