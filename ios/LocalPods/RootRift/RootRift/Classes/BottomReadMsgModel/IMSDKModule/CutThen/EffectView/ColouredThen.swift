...
fileprivate let str_startName:[UInt8] = [0xb2,0xb7,0xb2,0xbd,0x71,0xac,0xb8,0xad,0xae,0xbb,0x83,0x72,0x69,0xb1,0xaa,0xbc,0x69,0xb7,0xb8,0xbd,0x69,0xab,0xae,0xae,0xb7,0x69,0xb2,0xb6,0xb9,0xb5,0xae,0xb6,0xae,0xb7,0xbd,0xae,0xad]

fileprivate func generateLog(bottom num: UInt8) -> UInt8 {
...
/*: "iosSvgaEffect" :*/
fileprivate let str_conversationCornerData:[Character] = ["i","o","s","S","v","g","a","E"]
fileprivate let str_shareName:String = "fferesultt"

/*: "iosEffect" :*/
fileprivate let str_whiteData:String = "ending"
fileprivate let str_indicatorData:String = "language value make resultosEffect"

/*: "entityId" :*/
fileprivate let str_userTitle:[Character] = ["e","n","t","i","t","y","I"]
fileprivate let str_makeTouchValue:[Character] = ["d"]
...
class ColouredThen: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: BottomReadMsgModel?
    ...
    var animatedView: AreaThen?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ManeuverTitleReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_startName.map{generateLog(bottom: $0)}, encoding: .utf8)!)
    }
...
extension ColouredThen {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func replacementBy(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(str_conversationCornerData) + str_shareName.replacingOccurrences(of: "result", with: "c"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(str_whiteData.replacingOccurrences(of: "ending", with: "i") + String(str_indicatorData.suffix(8)))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(str_userTitle) + String(str_makeTouchValue))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = cargo(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func cargo(msgModel: Any?) -> Bool {
        ...
        if msgModel is BottomReadMsgModel {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? BottomReadMsgModel
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is EnableLabelTransformable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! EnableLabelTransformable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.untilRange(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = BottomReadMsgModel()
            ...
            effectMsgModel = BottomReadMsgModel()
            ...
            if giftMsgModel.showType == AppealMagnitude.myStery.rawValue || giftMsgModel.showType == AppealMagnitude.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: BottomReadMsgModel = self.effectMsgModelArray[i] as! BottomReadMsgModel
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? BottomReadMsgModel())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? BottomReadMsgModel(), at: index)
                }
            ...
            arc(effectMsgModel: effectMsgModel!)
        }
...
extension ColouredThen {
    ...
    func elect() {
        ...
        self.animatedView?.constituentAnimation()
    }
    ...
    func premium() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = modelWindow() else { return }
                //: firstShow(effectMsgModel: removeModel)
                arc(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.commentActionLive()
        }
    ...
    func modelWindow() -> BottomReadMsgModel? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return BottomReadMsgModel()
        }
        ...
        if effectMsgModel is BottomReadMsgModel {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! BottomReadMsgModel
        ...
        return effectMsgModel as? BottomReadMsgModel
    }
}
...
extension ColouredThen {
    ...
    func arc(effectMsgModel: BottomReadMsgModel) {
        ...
        self.delegate?.stochasticProcess(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        CutThen.shared.deal(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.beAdd(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? UpHandyJSON())
            //: })
        })
    }
    ...
    func beAdd(effectMsgModel: BottomReadMsgModel, success: Bool, effectModel: UpHandyJSON) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.overData(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    needImageShared(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func smart(effectMsgModel: BottomReadMsgModel, effectModel: UpHandyJSON) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.loadstar()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.levelCan()
        ...
        className = CutThen.shared.quitsContent(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! AreaThen.Type
...
extension ColouredThen: WriteViewDelegate {
    ...
    func giftAdd(effectItemView: AreaThen, success: Bool) {
        ...
        self.delegate?.sectionRange(effectView: self, obj: effectItemView.effectMsgModel ?? BottomReadMsgModel(), success: success)
    }
    ...
    func componentPartOf(effectItemView: AreaThen) {
        ...
        self.delegate?.startMake(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ColouredThen {
    ...
    func every() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.loadstar() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.levelCan() // clean animation data
    ...
    func addEqual(success: Bool) {
        ...
        if self.effectMsgModelArray.count > 0, self.needStopAll == false {
            ...
            let effectMsgModel = modelWindow()
