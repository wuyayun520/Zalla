...
fileprivate let str_itemText:[UInt8] = [0xd2,0xd5,0xd2,0xcf,0x93,0xd8,0xd4,0xdf,0xde,0xc9,0x81,0x92,0x9b,0xd3,0xda,0xc8,0x9b,0xd5,0xd4,0xcf,0x9b,0xd9,0xde,0xde,0xd5,0x9b,0xd2,0xd6,0xcb,0xd7,0xde,0xd6,0xde,0xd5,0xcf,0xde,0xdf]

/*: "%@.mp4" :*/
fileprivate let str_firstValue:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let str_equalContent:String = "hmomentp"
...
class UpPlayDelegate: AreaThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_itemText.map{$0^187}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(clickTo))
    ...
    override func shareLikeAnimating() {
        ...
        let path = CutThen.shared.pushModel(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? BottomReadMsgModel())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(str_firstValue)), path)
...
extension UpPlayDelegate: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((str_equalContent.replacingOccurrences(of: "moment", with: "tt"))) {
