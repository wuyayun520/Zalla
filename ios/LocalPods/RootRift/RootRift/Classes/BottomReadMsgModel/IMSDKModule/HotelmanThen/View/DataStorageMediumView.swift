...
fileprivate let str_whiteEqualValue:[Character] = ["T","R","T","C"," ","\u{89c6}","频","通","话","\u{5411}","S","D","K","发","送","采","集","\u{7684}","视","频","帧","\u{56fe}","\u{7247}"]
fileprivate let str_mightData:[Character] = [":"," "]

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let str_capValue:String = "LIVE 直in var normal false"
fileprivate let str_finishKeyValue:[Character] = ["播","\u{5411}","S","D","K","发","\u{9001}","采","\u{96c6}","的"]
fileprivate let str_checkData:String = "视\u{9891}帧图片: "
...
class DataStorageMediumView: UIView {
    // 直播推流类
...
extension DataStorageMediumView {
    /// 开启本地预览
    /// - Parameters:
    ///   - isSmallWindow: 是否自己在小窗口
    ///   - livePusher: 是否用tx推流
    //: func startPreView(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    func checkOutSub(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    ...
    func toQuantityUser(isOpen: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard kLet_errorDeviceAppValue == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension DataStorageMediumView: STFilterViewDelegete {
    // TRTC推流
    ...
    func stVideoBeautyView(_: STFilterView!, didProcessVideoSampleBuffer sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = memberImage(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                EnableReactiveCompatible.nameBy(msg: (String(str_whiteEqualValue) + String(str_mightData)) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
        ...
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = memberImage(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        EnableReactiveCompatible.nameBy(msg: (String(str_capValue.prefix(6)) + String(str_finishKeyValue) + str_checkData) + "\(image.size)")
                    }
                }
    ...
    func memberImage(_ imageBuffer: CVImageBuffer) -> UIImage? {
