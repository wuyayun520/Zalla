...
/*: "mf/call/GetUserInfo" :*/
fileprivate let str_contentData:String = "detail let raw for pathmf/ca"
fileprivate let str_viewValue:String = "count"
fileprivate let str_aspectValue:[Character] = ["l","/","G","e","t","U","s","e","r","I","n","f","o"]

/*: "uid" :*/
fileprivate let str_toValue:[UInt8] = [0xc5,0xd9,0xd4]

private func errorDate(lab num: UInt8) -> UInt8 {
    return num ^ 176
}

/*: "TRTC 随机视频匹配中." :*/
fileprivate let str_textData:[Character] = ["T"]
fileprivate let str_maleContent:String = "extension self modelRTC 随"

/*: "TRTC  :*/
fileprivate let str_userTitle:[Character] = ["T","R","T","C"," "]

/*: "发起" :*/
fileprivate let str_actionData:[UInt8] = [0xb7,0xb5,0xe8,0x91,0x8f,0xe5]

/*: "收到" :*/
fileprivate let str_defineTimeTitle:String = "\u{6536}到"

/*: 视频通话. logId: :*/
fileprivate let str_withHeadElseName:[Character] = ["视","频","通","话","."," ","l","o"]
fileprivate let str_dataValue:[Character] = ["g","I","d",":"]

/*: ." :*/
fileprivate let str_equalData:String = "."

/*: "s- :*/
fileprivate let str_colorContent:String = "s-cell date"

/*: - :*/
fileprivate let str_userValue:String = "app"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let str_labelData:[Character] = ["T","R","T","C"]
fileprivate let str_listViewControlData:String = " 视频通话"
fileprivate let str_norValue:String = "gId\u{ff1a}"

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let str_progressValue:[Character] = ["T","R","T","C"," ","\u{89c6}","频","通","话","\u{9519}","\u{8bef}"]
fileprivate let str_labelContent:String = "clear label bottom list. e"
fileprivate let str_addTitle:String = "path input varrrCode:"

/*: , errMsg: :*/
fileprivate let str_modeBottomText:String = "number table height to guard, err"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let str_pathLightValue:[Character] = ["T","R","T","C"," ","视","频","\u{901a}"]
fileprivate let str_endingTitle:[Character] = ["话","S","D","K","收"]
fileprivate let str_objectExistValue:[Character] = ["到","警","告","."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let str_requestTitle:String = ", warshare hidden"
fileprivate let str_limitValue:String = "label color array to iconMsg:"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let str_statusSpecialLetData:String = "TRTC window table"
fileprivate let str_blindTitle:String = "间."
fileprivate let str_directionName:String = "make equal res"

/*: , logId： :*/
fileprivate let str_tableFrameValue:String = ", loself fire info model value"

/*:  加入了房间" :*/
fileprivate let str_byTitle:String = " 加入了房detail"

/*:  离开了房间" :*/
fileprivate let str_textTitle:String = " 离开了房间"
...
enum QuantityRelationChatSituation {
    //: case Start
    case Start // 发起通话（非通话）
    //: case Recieved
...
class BottomChatConfig: NSObject {
    ...
    var remoteCameraStatus = GiftCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class TalkingCloudDelegate: NSObject {
...
extension TalkingCloudDelegate {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func misinformation(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = IndexRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(str_contentData.suffix(5)) + str_viewValue.replacingOccurrences(of: "count", with: "l") + String(str_aspectValue))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: str_toValue.map{errorDate(lab: $0)}, encoding: .utf8)!: uid]
...
extension TalkingCloudDelegate {
    ...
    func iconCurrent(_ frontCamera: Bool) {
        ...
        self.convert(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func noMethod(_ openCamera: Bool) {
        ...
        self.withSend(isOpen: openCamera)
    }
    ...
    func countercalate() {
        ...
        DispatchQueue.main.async {
            ...
            self.priceSmall(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension TalkingCloudDelegate {
    ...
    private func convert(front: Bool) {
    ...
    private func withSend(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_errorDeviceAppValue == true else {
            //: if isOpen {
            if isOpen {
...
extension TalkingCloudDelegate {
    ...
    func issueRenderMicroscopicMode() {
...
extension TalkingCloudDelegate {
    ...
    func barrelhouse() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(indexLive(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(audienceRequest(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(giftMin(notif:)),
    ...
    @objc private func hack(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func indexLive(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func audienceRequest(notif: Notification) {
    ...
    @objc func giftMin(notif: Notification) {
...
extension TalkingCloudDelegate {
    ...
    func equalMatch(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            EnableReactiveCompatible.nameBy(msg: (String(str_textData) + String(str_maleContent.suffix(5)) + "\u{673a}视频匹配\u{4e2d}."))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            EnableReactiveCompatible.nameBy(msg: (String(str_userTitle)) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (String(str_withHeadElseName) + String(str_dataValue)) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            priceSmall(smallWindow: false)
    ...
    func priceSmall(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_errorDeviceAppValue == true else {
    ...
    private func ok() {
        ...
        EnableReactiveCompatible.nameBy(msg: (String(str_labelData) + str_listViewControlData.capitalized + "\u{9500}毁. lo" + str_norValue) + "\(infoModel?.logId ?? 0).")
...
extension TalkingCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        EnableReactiveCompatible.nameBy(msg: (String(str_progressValue) + String(str_labelContent.suffix(3)) + String(str_addTitle.suffix(7))) + "\(errCode)" + (String(str_modeBottomText.suffix(5)) + "Msg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        EnableReactiveCompatible.nameBy(msg: (String(str_pathLightValue) + String(str_endingTitle) + String(str_objectExistValue)) + "\(warningCode)" + (String(str_requestTitle.prefix(5)) + "ning" + String(str_limitValue.suffix(4))) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: kLet_tableText, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        EnableReactiveCompatible.nameBy(msg: (String(str_statusSpecialLetData.prefix(5)) + "\u{89c6}频通话进入房" + str_blindTitle.capitalized + String(str_directionName.suffix(4)) + "ult:") + "\(result)" + (String(str_tableFrameValue.prefix(4)) + "gId\u{ff1a}") + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (str_byTitle.replacingOccurrences(of: "detail", with: "间")))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (str_textTitle.capitalized))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != WatercourseThen.share.loginUserMode.userID else { return }
        guard userId != WatercourseThen.share.loginUserMode.userID else { return }
