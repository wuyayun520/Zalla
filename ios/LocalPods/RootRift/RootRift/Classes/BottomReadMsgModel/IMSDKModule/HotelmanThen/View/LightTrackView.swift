...
fileprivate let str_labelValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class LightTrackView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_labelValue.reversed(), encoding: .utf8)!)
    }
...
extension LightTrackView {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: SubstituteItemView = subview as! SubstituteItemView
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: IndoorsPushManager.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    IndoorsPushManager.share.digitiserView(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func funcAppear(model: EnableLabelTransformable) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if bottomModel(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.dower(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.that(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: SubstituteItemView? = self.viewWithTag(tag) as! SubstituteItemView?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.structuralSociology(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: SubstituteItemView = self.viewWithTag(tag) as! SubstituteItemView
                ...
                subView1.addSourceWith(frame: frame)
    ...
    func bottomModel(giftMsgModel: EnableLabelTransformable, withGiftMsgModel: EnableLabelTransformable) -> (Bool) {
    ...
    func structuralSociology(giftMsgModel: EnableLabelTransformable, atIndex: Int) {
        //: var startX = 0
        var startX = 0
        //: if LanguageManager.shared.direction == .rightToLeft {
        if FactoryShowThen.shared.direction == .rightToLeft {
            //: startX = -Int(ScreenWidth)-15
            startX = -Int(kLet_scaleName) - 15
        }
        //: let frame = CGRect.init(x: startX, y: atIndex * 60, width: Int(ScreenWidth), height: 60)
        let frame = CGRect(x: startX, y: atIndex * 60, width: Int(kLet_scaleName), height: 60)
        //: let subView =  TalkingGiftTrackItemView.init(frame: frame)
        let subView = SubstituteItemView(frame: frame)
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.that(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.stopDate(model: giftMsgModel, animate: true)
    }
}
