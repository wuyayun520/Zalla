...
fileprivate let str_viewTitleContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let str_maxData:String = "moofe"
...
class PerformanceReactiveCompatible: UIView {
    ...
    var danumuMsgArray: [RequestMeasurable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [IndoorsReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<RequestMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_viewTitleContent.reversed(), encoding: .utf8)!)
    }
    ...
    func logSubReceive(_ danumuMsgModel: RequestMeasurable?) {
    ...
    @objc func influence(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(influence(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension PerformanceReactiveCompatible: UIGestureRecognizerDelegate {
    ...
    func bullSEye(for danumuMsgModel: RequestMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = IndoorsReactiveCompatible(model: danumuMsgModel)
        ...
        var frame = CGRect(x: kLet_scaleName, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if FactoryShowThen.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = kLet_scaleName
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_maxData.replacingOccurrences(of: "of", with: "v")))
    ...
    func modelto(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                bullSEye(for: model, isFirst: isFirst)
