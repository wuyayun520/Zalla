...
fileprivate let str_showData:[UInt8] = [0xa0,0xa7,0xa0,0xbd,0xe1,0xaa,0xa6,0xad,0xac,0xbb,0xf3,0xe0,0xe9,0xa1,0xa8,0xba,0xe9,0xa7,0xa6,0xbd,0xe9,0xab,0xac,0xac,0xa7,0xe9,0xa0,0xa4,0xb9,0xa5,0xac,0xa4,0xac,0xa7,0xbd,0xac,0xad]

private func dataConverter(gift num: UInt8) -> UInt8 {
    return num ^ 201
}

/*: "View read receipts?" :*/
fileprivate let str_dataMakeName:String = "title self name withView"
fileprivate let str_effectData:String = "color hidden intimate rece"
fileprivate let str_playerViewContent:String = "ipts?log height"

/*: "text" :*/
fileprivate let str_playTitle:String = "modelxt"

/*: "color" :*/
fileprivate let str_tagData:[UInt8] = [0x0,0xc,0xf,0xc,0x11]

/*: "<br/>" :*/
fileprivate let str_colorText:[Character] = ["<","b","r","/",">"]

/*: "\n" :*/
fileprivate let str_rankTitle:[Character] = ["\n"]

/*: "<br />" :*/
fileprivate let str_sizeName:[Character] = ["<","b","r"," ","/",">"]

/*: "<xml>%@</xml>" :*/
fileprivate let str_viewShareValue:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let str_modelTitle:[Character] = ["h","r","e","f"]

/*: "type" :*/
fileprivate let str_cellData:[UInt8] = [0xde,0xd3,0xda,0xcf]

private func cookieMessage(area num: UInt8) -> UInt8 {
    return num ^ 170
}

/*: "txt" :*/
fileprivate let str_maxData:String = "partyt"

/*: "padding" :*/
fileprivate let str_oldName:String = "paddcolorg"

/*: "size" :*/
fileprivate let str_modelMessageMakeData:[Character] = ["s","i","z","e"]

/*: "maxFont" :*/
fileprivate let str_sectionName:[Character] = ["m"]
fileprivate let str_gestureValue:String = "height self toaxFont"

/*: "link" :*/
fileprivate let str_atTitle:[Character] = ["l","i","n","k"]

/*: "PingFangSC-Regular" :*/
fileprivate let str_voiceName:[Character] = ["P","i","n","g","F","a","n","g"]
fileprivate let str_equalRejectValue:[Character] = ["S","C","-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let str_contentData:[UInt8] = [0xf5,0xf9,0xf8,0xe2,0xf3,0xf8,0xe2]

/*: "%@" :*/
fileprivate let str_titleData:String = "%@"
...
//: @objcMembers public class HandbillThen: TalkingChatMsgBaseCellData {
@objcMembers public class HandbillThen: WriteRequestCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_showData.map{dataConverter(gift: $0)}, encoding: .utf8)!)
    }
    ...
    override public func contentSize() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(str_dataMakeName.suffix(4)) + " read" + String(str_effectData.suffix(5)) + String(str_playerViewContent.prefix(5))).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: kLet_scaleName, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: ListMacroDefine.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: ListMacroDefine.width() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = appearanceInSkinny(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
//: extension HandbillThen {
extension HandbillThen {
    //: func formatMessageToStringM(info: WithLayerModel) -> NSMutableAttributedString {
    func appearanceInSkinny(info: WithLayerModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return pokingIndex(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func pokingIndex(tips: String) -> NSMutableAttributedString {
        ...
        _ = desginBy(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: ListMacroDefine.getScreenWidth()-50, para: para)
        self.yyLayout = application(array: attrbutedArray, displayMaxWidth: ListMacroDefine.width() - 50, para: para)
    ...
    func desginBy(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = part(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(str_playTitle.replacingOccurrences(of: "model", with: "te"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!, (str_playTitle.replacingOccurrences(of: "model", with: "te"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = nameUpLevelColor(content: text, color: textColor)
    ...
    func part(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(str_viewShareValue)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            neighbour(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(str_playTitle.replacingOccurrences(of: "model", with: "te")): newSource, String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func neighbour(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(String(str_modelTitle))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(String(str_modelTitle))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                neighbour(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(str_playTitle.replacingOccurrences(of: "model", with: "te"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func application(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = labelStyle(array: array, paragraphStyle: para)
    ...
    func labelStyle(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: str_cellData.map{cookieMessage(area: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (str_maxData.replacingOccurrences(of: "party", with: "tx")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = self.dict(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (str_oldName.replacingOccurrences(of: "color", with: "in")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(String(str_modelMessageMakeData))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(str_sectionName) + String(str_gestureValue.suffix(6)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!] as? UIColor)
    ...
    func desgin(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(String(str_modelMessageMakeData))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(str_sectionName) + String(str_gestureValue.suffix(6)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func dict(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: str_cellData.map{cookieMessage(area: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (String(str_atTitle)) && type != (str_maxData.replacingOccurrences(of: "party", with: "tx")) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = bankRequest()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(String(str_modelMessageMakeData))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(str_voiceName) + String(str_equalRejectValue)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: str_contentData.map{$0^150}, encoding: .utf8)!] != nil) ? dict[String(bytes: str_contentData.map{$0^150}, encoding: .utf8)!] : ""
    ...
    func bankRequest() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.rugular(fontSize: 15)
    ...
    func nameUpLevelColor(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return endText(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: str_cellData.map{cookieMessage(area: $0)}, encoding: .utf8)!: (str_maxData.replacingOccurrences(of: "party", with: "tx")),
                //: "content": content,
                String(bytes: str_contentData.map{$0^150}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: str_tagData.map{$0^99}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func endText(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
