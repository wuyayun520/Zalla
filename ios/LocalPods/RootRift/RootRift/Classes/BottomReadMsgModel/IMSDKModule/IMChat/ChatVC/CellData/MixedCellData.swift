...
fileprivate let str_fillData:[UInt8] = [0x9d,0x9a,0x9d,0x80,0xdc,0x97,0x9b,0x90,0x91,0x86,0xce,0xdd,0xd4,0x9c,0x95,0x87,0xd4,0x9a,0x9b,0x80,0xd4,0x96,0x91,0x91,0x9a,0xd4,0x9d,0x99,0x84,0x98,0x91,0x99,0x91,0x9a,0x80,0x91,0x90]

private func quoteUser(position num: UInt8) -> UInt8 {
    return num ^ 244
}

/*: "[未知消息]" :*/
fileprivate let str_voiceContent:[Character] = ["[","未","知","\u{6d88}","息","]"]

/*: "extra" :*/
fileprivate let str_makeValue:String = "EXTRA"

/*: "</at> " :*/
fileprivate let str_labelCostManagerName:String = "color equal size type</at>"
fileprivate let str_statusName:String = "collection"

/*: "#0084FF" :*/
fileprivate let str_countData:String = "size type white guard#0084FF"

/*: "#FFEE4B" :*/
fileprivate let str_giftValue:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let str_tingData:String = "<at>@height height layer"
fileprivate let str_dataTitle:[Character] = ["+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let str_leadingVersionName:[Character] = ["j","u","m","p","K","e","y"]

/*: "user" :*/
fileprivate let str_countValue:String = "USER"

/*: "uid" :*/
fileprivate let str_bottomModelValue:[UInt8] = [0x74,0x68,0x63]

fileprivate func tingTop(cell num: UInt8) -> UInt8 {
...
/*: "#16D073" :*/
fileprivate let str_partyName:[Character] = ["#","1","6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let str_textData:String = "#F956open not style view"
fileprivate let str_equalText:[Character] = ["4","4"]
...
//: @objcMembers public class MixedCellData: TalkingChatMsgBaseCellData {
@objcMembers public class MixedCellData: WriteRequestCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_fillData.map{quoteUser(position: $0)}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: FrameJsonModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (String(str_voiceContent))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = tied(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func tied(msgInfoModel: CornerModel) -> NSMutableAttributedString {
        ...
        guard dic[(str_makeValue.lowercased())] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.ending() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(str_countData.suffix(7))))! : UIColor(hex: (String(str_giftValue)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(str_tingData.prefix(5)) + "[\\S\\s]" + String(str_dataTitle)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo["jumpKey"] == "user" {
                ...
                uid = jumpInfo[String(bytes: str_bottomModelValue.map{tingTop(cell: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: str_bottomModelValue.map{tingTop(cell: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func bringHome() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(str_partyName)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(str_textData.prefix(5)) + String(str_equalText)))!
        }
