...
fileprivate let str_colorText:[UInt8] = [0x78,0x7f,0x78,0x65,0x39,0x72,0x7e,0x75,0x74,0x63,0x2b,0x38,0x31,0x79,0x70,0x62,0x31,0x7f,0x7e,0x65,0x31,0x73,0x74,0x74,0x7f,0x31,0x78,0x7c,0x61,0x7d,0x74,0x7c,0x74,0x7f,0x65,0x74,0x75]

/*: "icon_chat_lock" :*/
fileprivate let str_groupText:String = "make with viewicon_"
fileprivate let str_postTitle:[Character] = ["l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let str_helloBottomText:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u"]
fileprivate let str_labLabelKeyData:[Character] = ["r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let str_discussValue:String = "I"
fileprivate let str_managerText:[Character] = ["c","o","n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let str_statusValue:String = "value letAlrea"
...
class ViewMessageCell: KeyMessageCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_colorText.map{$0^17}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        //: imag.alpha = 0.9
        imag.alpha = 0.9
        //: imag.image = UIImage.withView(name: "icon_chat_lock")
        imag.image = UIImage.withView(name: (String(str_groupText.suffix(5)) + "chat_" + String(str_postTitle)))
    ...
    lazy var burnedImagV: MediaControl = {
        ...
        imgV.setBackgroundImage(UIImage.withView(name: (String(str_helloBottomText) + String(str_labLabelKeyData))), for: .normal)
        //: imgV.setImage(UIImage.withView(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.withView(name: (str_discussValue.lowercased() + String(str_managerText))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(str_statusValue.suffix(5)) + "dy burned").localized, for: .normal)
...
extension ViewMessageCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: guard let imageCellData = data as? AutomaticallyCellData else { return }
        guard let imageCellData = data as? AutomaticallyCellData else { return }

        //: self.photoView.backgroundColor = .white
        self.photoView.backgroundColor = .white
        //: let intimatePhoto: LabModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        let intimatePhoto: LabModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            progress(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            progress(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: AutomaticallyCellData, urlStr: String?, isMosaic: Bool) {
    func progress(data: AutomaticallyCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.number(image, owner: 90)
                        self.photoView.image = UIImage.number(image, owner: 90)
                        //: } else {
                    } else {
            ...
            large(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.ofData(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func large(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.objectTime(urlStr: urlStr) { [weak self] image in
