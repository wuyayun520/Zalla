...
fileprivate let str_fromValue:[UInt8] = [0x7f,0x78,0x7f,0x62,0x3e,0x75,0x79,0x72,0x73,0x64,0x2c,0x3f,0x36,0x7e,0x77,0x65,0x36,0x78,0x79,0x62,0x36,0x74,0x73,0x73,0x78,0x36,0x7f,0x7b,0x66,0x7a,0x73,0x7b,0x73,0x78,0x62,0x73,0x72]

private func fileSend(bottom num: UInt8) -> UInt8 {
    return num ^ 22
}

/*: "icon_coinxxx" :*/
fileprivate let str_requestShadowValue:[Character] = ["i","c","o","n","_"]
fileprivate let str_releaseLeadingText:[Character] = ["c","o","i","n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let str_titleName:String = "icon_gmini moment icon section"
fileprivate let str_kitPartnerName:String = "num_bglet self return image cos"

/*: "x1" :*/
fileprivate let str_infoData:[Character] = ["x","1"]

/*: " x :*/
fileprivate let str_statusValue:[Character] = [" ","x"]
...
class BestowmentGiftCell: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = AdMeasurable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_fromValue.map{fileSend(bottom: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.conversationSize(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: MediaControl = {
        //: let btn = TalkingButton.init()
        let btn = MediaControl()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.conversationSize(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.withView(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.withView(name: (String(str_requestShadowValue) + String(str_releaseLeadingText))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let img = UIImageView()
        //: let imag = UIImage.withView(name: "icon_gift_num_bg")
        let imag = UIImage.withView(name: (String(str_titleName.prefix(6)) + "ift_" + String(str_kitPartnerName.prefix(6))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.conversationSize(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension BestowmentGiftCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func magnitudeimateReceive(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.smallFor().cgColor
    ...
    func intervaleraction(number: Int) {
    ...
    func use(currenmodel: AdMeasurable, giftType: UnknownRepresentable) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.ofData(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.ofData(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.conversationSize(type: .Regular, fontSize: 11),
...
extension BestowmentGiftCell {
    ...
    func needUi() {
        ...
        bgView.backgroundColor = UIColor.commensurateColor()
