...
fileprivate let str_videoLockValue:String = "\u{8bbe}置普通"
fileprivate let str_toolValue:String = "字颜色时 "
fileprivate let str_delayTitle:String = "B空间的颜\u{8272}值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let str_effectData:[Character] = ["设","置","\u{9009}","中","状","态","的","文","字","颜","色","时"," ","请","使","用","R","G"]
fileprivate let str_inputNearName:String = "min"
fileprivate let str_equalValue:[Character] = ["空","间","\u{7684}","\u{989c}","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_sizeValue:[UInt8] = [0xa7,0xac,0xa7,0xb2,0x66,0xa1,0xad,0xa2,0xa3,0xb0,0x78,0x67,0x5e,0xa6,0x9f,0xb1,0x5e,0xac,0xad,0xb2,0x5e,0xa0,0xa3,0xa3,0xac,0x5e,0xa7,0xab,0xae,0xaa,0xa3,0xab,0xa3,0xac,0xb2,0xa3,0xa2]

fileprivate func scaleFrom(by num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let str_modelContent:[Character] = ["设","置","\u{7684}","下","标","不","合","法","!","!"]

/*: "Bags" :*/
fileprivate let str_valueTitle:[Character] = ["B","a","g","s"]
...
open class WriteSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: HeadAddSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.happening(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((str_videoLockValue + "状态的文" + str_toolValue.capitalized + "请使用RG" + str_delayTitle))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.happening(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(str_effectData) + str_inputNearName.replacingOccurrences(of: "min", with: "B") + String(str_equalValue)))
        }
    ...
    fileprivate func happening(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: HeadAddSegmentStyle, titles: [String]) {
    ...
    @objc func tableRange(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? TitleLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_sizeValue.map{scaleFrom(by: $0)}, encoding: .utf8)!)
    }
}
...
public extension WriteSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func anyAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(str_modelContent)))
    ...
    func researchLaboratory(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension WriteSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func lifestyle() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = TitleLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.tableRange(_:)))
            ...
            if title == (String(str_valueTitle)).localized {
    ...
    fileprivate func approximately() {
        ...
        if let firstLabel = labelsArray[0] as? TitleLabel {
    ...
    fileprivate func setupTimePaste() {
    ...
    public func allFinish(_ currentIndex: Int) {
