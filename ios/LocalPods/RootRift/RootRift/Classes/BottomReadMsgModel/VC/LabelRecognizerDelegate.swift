...
fileprivate let str_viewValue:[UInt8] = [0x11,0x16,0x11,0x1c,0xd0,0xb,0x17,0xc,0xd,0x1a,0xe2,0xd1,0xc8,0x10,0x9,0x1b,0xc8,0x16,0x17,0x1c,0xc8,0xa,0xd,0xd,0x16,0xc8,0x11,0x15,0x18,0x14,0xd,0x15,0xd,0x16,0x1c,0xd,0xc]

fileprivate func forError(mini num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let str_coverData:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","g","i","f","t","b","g"]

/*: "Send" :*/
fileprivate let str_itemData:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let str_cancelMajorityText:[Character] = ["n","a","v","_","b","a","c","k","_","b","l"]
fileprivate let str_popValue:String = "message"
fileprivate let str_titleText:[Character] = ["c","k","_","n","o","r"]

/*: "toUid" :*/
fileprivate let str_labData:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let str_upTitle:[Character] = ["g","i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let str_norValue:String = "user colorgiftNu"
fileprivate let str_normalName:String = "corner"

/*: "unlockMsgId" :*/
fileprivate let str_endValue:String = "unlonce"
fileprivate let str_errorData:[Character] = ["I","d"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let str_actualData:[UInt8] = [0x6b,0x63,0x6f,0x6c,0x6e,0x75,0x20,0x6f,0x74,0x20,0x29,0x73,0x6e,0x69,0x6f,0x63,0x20,0x64,0x6c,0x6f,0x47,0x20,0x40,0x25,0x28,0x20,0x40,0x25,0x20,0x61,0x20,0x64,0x65,0x65,0x4e]

/*: "icon_photo_lock" :*/
fileprivate let str_prepareContent:String = "icbarn"
fileprivate let str_upSizeValue:String = "_lockview submit view as action"

/*: "Click on the screen to view 15s" :*/
fileprivate let str_countData:[UInt8] = [0x6b,0x44,0x41,0x4b,0x43,0x8,0x47,0x46,0x8,0x5c,0x40,0x4d,0x8,0x5b,0x4b,0x5a,0x4d,0x4d,0x46,0x8,0x5c,0x47,0x8,0x5e,0x41,0x4d,0x5f,0x8,0x19,0x1d,0x5b]

/*: "icon_photo_burn" :*/
fileprivate let str_modelValue:String = "ivalue"
fileprivate let str_contentNowValue:String = "type previouson_p"

/*: "Already burned" :*/
fileprivate let str_textValue:[Character] = ["A","l","r","e","a","d"]
fileprivate let str_tapText:[Character] = ["y"," ","b","u","r","n","e","d"]
...
//: typealias IntimatePhotoBlock = (_ model: LabModel) -> Void
typealias IntimatePhotoBlock = (_ model: LabModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class LabelRecognizerDelegate: MusicViewController {
    ...
    //: private var imageTotalData = AutomaticallyCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = AutomaticallyCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = LabModel()
    private var photoModel = LabModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: AutomaticallyCellData, toUid: String) {
    init(imageCellData: AutomaticallyCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(WatercourseThen.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(WatercourseThen.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_viewValue.map{forError(mini: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.withView(name: (String(str_coverData)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.ofData(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(str_itemData)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(colorRecord), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.withView(name: (String(str_cancelMajorityText) + str_popValue.replacingOccurrences(of: "message", with: "a") + String(str_titleText))).withRenderingMode(.alwaysTemplate)
...
extension LabelRecognizerDelegate {
    ...
    func discharge(giftId: String, giftNum: String) {
        ...
        params[(String(str_labData))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(str_upTitle))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(str_norValue.suffix(6)) + str_normalName.replacingOccurrences(of: "corner", with: "m"))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(str_endValue.replacingOccurrences(of: "once", with: "o") + "ckMsg" + String(str_errorData))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension LabelRecognizerDelegate {
    ...
    @objc private func colorRecord() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        discharge(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func comprehensibleGesture() {
        ...
        self.submitTo(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            AtAdView.toAction()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.paradigm(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension LabelRecognizerDelegate {
    ...
    private func iconAllow() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: str_actualData.reversed(), encoding: .utf8)!.alongNext(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.withView(name: (str_prepareContent.replacingOccurrences(of: "bar", with: "o") + "_photo" + String(str_upSizeValue.prefix(5))))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: str_countData.map{$0^40}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(comprehensibleGesture))
                ...
                self.intimateStatusImgV.image = UIImage.withView(name: (str_modelValue.replacingOccurrences(of: "value", with: "c") + String(str_contentNowValue.suffix(4)) + "hoto_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(str_textValue) + String(str_tapText)).localized
            }
        ...
        self.submitTo(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func submitTo(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
