...
/*: "gifts" :*/
fileprivate let str_submitNumbName:String = "gofts"

/*: "vipGifts" :*/
fileprivate let str_coverText:String = "file"
fileprivate let str_detailNeedKeyData:[Character] = ["i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let str_cellVisualLetTitle:[Character] = ["p","a","c","k"]
fileprivate let str_soundVoiceValue:[Character] = ["G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let str_targetText:String = "progress log leadinggiftSend"
fileprivate let str_indexText:String = "load app tool text selfItem"
...
class ExaggerateReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [UnknownRepresentable: [Any]]]()
    // 数据锁
    ...
    private var giftType = VoiceBecomeSchemaConvertible.normal

    //: static let share = TalkingChatGiftManager()
    static let share = ExaggerateReactiveCompatible()
...
extension ExaggerateReactiveCompatible {
    ...
    func priceTagToImage(type: VoiceBecomeSchemaConvertible = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        ExaggerateReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[UnknownRepresentable.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            ExaggerateReactiveCompatible.share.common(completion: { isSucceed in
            ...
            ExaggerateReactiveCompatible.share.atAccumulation(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ExaggerateReactiveCompatible.share.streetwiseCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ExaggerateReactiveCompatible.share.removeEqualCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func removeEqualCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ExaggerateReactiveCompatible.share.giftType
        //: HumanHeadRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        HumanHeadRequestTool.failKey(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [UnknownRepresentable: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(str_submitNumbName.replacingOccurrences(of: "of", with: "if"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AdMeasurable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[UnknownRepresentable.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(str_coverText.replacingOccurrences(of: "file", with: "v") + String(str_detailNeedKeyData))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AdMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[UnknownRepresentable.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_cellVisualLetTitle) + String(str_soundVoiceValue))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AdMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[UnknownRepresentable.Bags] = bagsArr
                }
                ...
                var firstModel = LogTransformable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(str_targetText.suffix(8)) + String(str_indexText.suffix(4)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<LogTransformable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func common(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ExaggerateReactiveCompatible.share.giftType
        //: HumanHeadRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        HumanHeadRequestTool.textImage(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func natalDay(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: HumanHeadRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        HumanHeadRequestTool.noExtract(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<AllCellTransformable>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func atAccumulation(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = VoiceBecomeSchemaConvertible.live
        //: HumanHeadRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        HumanHeadRequestTool.completionAcquisitionTypeShackList(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func streetwiseCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = VoiceBecomeSchemaConvertible.party
        //: HumanHeadRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        HumanHeadRequestTool.shadow(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func quantityDataType(perPageCount: Int, dataType: UnknownRepresentable) -> Int {
    ...
    func blistery(dataType: UnknownRepresentable) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func noneBlock() -> [Any] {
        ...
        if ExaggerateReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if ExaggerateReactiveCompatible.share.giftType == .live || ExaggerateReactiveCompatible.share.giftType == .party {
    ...
    func viewArr(dataType: UnknownRepresentable, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension ExaggerateReactiveCompatible {
    ...
    private func dominantNotification() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(endeavor),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: kLet_systemScreenTitle,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    @objc private func endeavor() {
        //: gift_clearMemoryCache(showRed: true)
        starNote(showRed: true)
    }
}
...
extension ExaggerateReactiveCompatible {
    ...
    func starNote(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        ExaggerateReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
