...
fileprivate let str_tableValue:[UInt8] = [0xaf,0xa8,0xaf,0xb2,0xee,0xa5,0xa9,0xa2,0xa3,0xb4,0xfc,0xef,0xe6,0xae,0xa7,0xb5,0xe6,0xa8,0xa9,0xb2,0xe6,0xa4,0xa3,0xa3,0xa8,0xe6,0xaf,0xab,0xb6,0xaa,0xa3,0xab,0xa3,0xa8,0xb2,0xa3,0xa2]

private func sizeModel(key num: UInt8) -> UInt8 {
    return num ^ 198
}

/*: "#333333" :*/
fileprivate let str_miniTitle:[Character] = ["#","3","3","3","3","3","3"]

/*: "#999999" :*/
fileprivate let str_hiddenTitle:[Character] = ["#","9","9","9","9","9","9"]

/*: "isCupid" :*/
fileprivate let str_colorPlayerData:[Character] = ["i","s","C","u","p"]
fileprivate let str_cropModeValue:String = "make"

/*: "autoGreetAuth" :*/
fileprivate let str_appObjectContent:[Character] = ["a","u","t","o","G"]
fileprivate let str_groupValue:String = "reetAuthlabel back first"

/*: "isLeaderboardAnonymous" :*/
fileprivate let str_topValue:String = "type let self inisLead"
fileprivate let str_managerRequestValue:String = "ardAnoview source position"

/*: "isSameGendered" :*/
fileprivate let str_pageTitle:[Character] = ["i","s","S","a","m","e","G","e","n"]
fileprivate let str_blockData:String = "pointerepoint"

/*: "type" :*/
fileprivate let str_sharedModeTextValue:String = "tyburne"

/*: "value" :*/
fileprivate let str_colorName:String = "vastyleue"
...
class SubView: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(withIndex), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_tableValue.map{sizeModel(key: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(str_miniTitle)))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(price), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
...
extension SubView {
    ...
    func errorStart(titile: String, row: Int, isLast: Bool) {
...
//        if titile == SettingsDefaultsSerializable.General.rawValue {
//            ColorPermissionTool.checkPushNotification { isShow in
...
//        }
    ...
    @objc func price() {
        ...
        if titleBLB.text == DecisionMakerCollection.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == DecisionMakerCollection.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            configDoing(type: (String(str_colorPlayerData) + str_cropModeValue.replacingOccurrences(of: "make", with: "id")), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == DecisionMakerCollection.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            configDoing(type: (String(str_appObjectContent) + String(str_groupValue.prefix(8))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == DecisionMakerCollection.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            configDoing(type: (String(str_topValue.suffix(6)) + "erbo" + String(str_managerRequestValue.prefix(6)) + "nymous"), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == DecisionMakerCollection.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            configDoing(type: (String(str_pageTitle) + str_blockData.replacingOccurrences(of: "point", with: "d")), value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func configDoing(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(str_sharedModeTextValue.replacingOccurrences(of: "burn", with: "p"))] = type
        //: dict["value"] = va
        dict[(str_colorName.replacingOccurrences(of: "style", with: "l"))] = va

        //: ProgressHUD.show()
        AtAdView.sizeUp()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        AnteriorNarisRequestTool.appMoreLive(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
            AtAdView.toAction()
            //: if succeed {
            if succeed {
    ...
    @objc func withIndex(notification _: NSNotification) {
//        if titleBLB.text == SettingsDefaultsSerializable.General.rawValue {
//            ColorPermissionTool.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
