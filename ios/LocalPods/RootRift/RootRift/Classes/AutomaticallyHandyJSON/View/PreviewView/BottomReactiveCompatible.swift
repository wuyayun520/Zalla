...
fileprivate let str_modelTitle:[UInt8] = [0x30,0x35,0x30,0x3b,0xef,0x2a,0x36,0x2b,0x2c,0x39,0x1,0xf0,0xe7,0x2f,0x28,0x3a,0xe7,0x35,0x36,0x3b,0xe7,0x29,0x2c,0x2c,0x35,0xe7,0x30,0x34,0x37,0x33,0x2c,0x34,0x2c,0x35,0x3b,0x2c,0x2b]

fileprivate func rootView(main num: UInt8) -> UInt8 {
...
/*: "icon_male_default" :*/
fileprivate let str_transformTitle:[UInt8] = [0x5e,0x54,0x58,0x59,0x68,0x5a,0x56,0x5b,0x52,0x68,0x53,0x52,0x51,0x56,0x42,0x5b,0x43]

/*: "icon_female_default" :*/
fileprivate let str_equalName:String = "icon_feimage if let current feed"
fileprivate let str_listValue:String = "mmodelle"
fileprivate let str_toolData:String = "result content_def"

/*: "level_%@" :*/
fileprivate let str_canUserContent:String = "LEV"
fileprivate let str_inviteTitle:String = "to succeedel_%@"
...
// __CLOSE_PRINT__
//
//  BottomReactiveCompatible.swift
//  AbroadTalking
//
...
class BottomReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_modelTitle.map{rootView(main: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .conversationSize(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .conversationSize(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension BottomReactiveCompatible {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func adjust(model: WithHandyJSON) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.appellation())
        ...
        let imgStr = model.sex == PushDownStackScalarLiteral.male.rawValue ? String(bytes: str_transformTitle.map{$0^55}, encoding: .utf8)! : (String(str_equalName.prefix(7)) + str_listValue.replacingOccurrences(of: "model", with: "a") + String(str_toolData.suffix(4)) + "ault")
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (str_canUserContent.lowercased() + String(str_inviteTitle.suffix(5))), model.level)
